...
const Details = () => {
  ...
  return (
    <section ref={ref} className="bg-white text-[#00A3FF] py-16 px-8">
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 text-center">
        {stats.map((item, index) => (
          <div key={index} className="flex flex-col items-center">
         <Reveal>  <div className="bg-white/20 rounded-full p-6 mb-4">{item.icon}</div></Reveal> 
        <Reveal>     <h2 className="text-4xl font-bold">
              {inView ? (
          ...
          <Reveal>   <div className="text-black   mt-4 border border-white/50 rounded-md px-6 py-2 font-semibold text-lg">
              {item.label}
            </div></Reveal> 
          </div>
        ))}
