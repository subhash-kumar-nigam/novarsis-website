                ...
                <tbody>
                    {cartData?.data.length ? <> {
                        cartData?.data.map((item, i) => {
                            return <tr key={i}>
                                <th>{i + 1}</th>
                                <td><a href={`${process.env.REACT_APP_BACKEND}resume/${item.resume}`}>View Resume</a></td>
                                <td>{item.jobrole}</td>
                                <td>{item.name}</td>
                                <td>{item.email}</td>
                                <td>{item.mobile}</td>
                                <td>{item.subject}</td>
                                <td>{item.message}</td>
                                <td><button className='btn removebtn' onClick={() => dispatch(removeCareer(item.id))}>Remove</button></td>
                            </tr>
                        })
                    }
                    </>
