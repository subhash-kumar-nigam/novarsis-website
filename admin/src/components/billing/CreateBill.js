...
const CreateBill = () => {
  ...
  const changeQuantity = (e, id) => {
    ...
    if (!isNaN(newQuantity) && newQuantity >= 0) {
      setProdata((prevProData) => prevProData.map((item) => (item.id === id ? { ...item, quantity: newQuantity } : item)));
    }
  };
  ...
  const downloadPdf = () => {
    ...
    printWindow.document.write(
      '<div class="container"><div class="row"><div class="col-6"><img class="mt-3" src="http://localhost:3000/static/media/mylogo.8783c030dbdba0f91f99.png" height="100"/></div><div class="col-6 align-item-right"><p>Yashsoft Solution</p></div></div></div>'
    );
    // printWindow.document.write('');
    ...
    <div className="container-fluid">
      ...
      <div className="row my-5">
        <div className="col-sm-5">
          <button className="btn btn-success w-100" onClick={initializeScanner}>
            Scan Item
          </button>
        </div>
        ...
        <div className="col-sm-6">
          <input
            ...
            onChange={(e) => {
              e.target.value.length ? dispatch(searchProductByName(e.target.value)) : dispatch(searchProductByName(null));
            }}
          />
            ...
            <div style={{ height: 200, backgroundColor: '#e2d9d9' }}>
              {cartData.data &&
                cartData.data.map((item, key) => {
                  return (
                    <button key={key} className="m-3 p-3 bg-white" onClick={() => addItemManualy(item)}>
                      {item.name} || {item.unit}
                    </button>
                  );
                })}
      ...
      <div className="row">
        ...
        <input
          ...
          className="form-control"
          onChange={(e) => {
            e.target.value ? dispatch(searchByMobile(e.target.value)) : dispatch(searchByMobile(null));
          }}
        />
        {customer.data.length ? (
          <div className="w-100" style={{ backgroundColor: '#e2d9d9' }}>
            {customer.data &&
              customer.data.map((item, key) => {
                return (
                  <button key={key} className="m-3 p-3 bg-white" onClick={() => addCustomerBymobile(item)}>
            ...
            {proData.length ? (
              proData.map((item, key) => (
                <tr key={item.id}>
                  ...
                  <td>{item.name}</td>
                  <td>
                    <input type="number" onChange={(e) => changeQuantity(e, item.id)} value={item.quantity} />
                  </td>
                  <td>{item.sprice}</td>
