          ...
          <div className="receipt-main col-xs-10 col-sm-10 col-md-12 col-xs-offset-1 col-sm-offset-1 col-md-offset-3">
            <div className="row">
              <div className="col-sm-6 col-md-6">
                <div className="receipt-left">
                  ...
                  <p><strong> Customer Name :</strong> {cartData.data?.customerData?.name}</p>
                  <p><strong>Mobile :</strong> {cartData.data?.customerData?.mobile}</p>
                  <p><strong>Address :</strong> {cartData.data?.order?.shipping_address}</p>
                  <p><strong>Status :</strong> {cartData.data?.order?.status}</p>
                </div>
              </div>
            ...
            <div>
              <table className="table table-bordered">
                ...
                <tbody>
                  {cartData.data?.itemData1 ? cartData.data?.itemData1.map((item, key) => (
                    <tr key={key}>
                      <td className="col-md-4">{item.product.name}</td>
                      <td className="col-md-3">{item.quantity}</td>
                      <td className="col-md-2">{item.price}</td>
                      <td className="col-md-3"><i className="fa fa-inr"></i> {item.subtotal}/-</td>
                    </tr>
                  )) : ''}
                  <tr></tr>
                  <tr>
                    <td className="text-right" colSpan={3}><h5><strong>Total: </strong></h5></td>
                    <td className="text-left text-danger"><h5><strong><i className="fa fa-inr"></i> {calculateTotal()}/-</strong></h5 ></td>
                  </tr>
                </tbody>
              </table>
            </div>
            ...
            <div className="row">
              <div className="receipt-header receipt-header-mid receipt-footer">
                <div className="col-xs-8 col-sm-8 col-md-12 text-left">
                  <div className="receipt-right">
                    <p><b>Date :</b> {cartData.data?.itemData1?.[0]?.createdAt}</p>
